    ...
    <>
      {displayBaseHint && (
        <div className="feeToolTip feeToolTipBase cardShow">
          The current base fee is added with a buffer to cover the next 3
          blocks, any amount greater than your block's base fee is refunded
        </div>
      )}
      {displayPriorityHint && (
        <div className="feeToolTip feeToolTipPriority cardShow">
          A priority tip paid to validators is added to incentivize quick
          inclusion of your transaction into a block
        </div>
      )}
...
class GasSummaryComponent extends Component {
  ...
  txEstimates(type, id, gasPrice, calculatedFees, currentSymbol) {
    ...
    if (id === 10) {
      ...
      const ethPriceLevels = this.store(
      ...
      const optimismEstimate = (l1Limit, l2Limit) => {
        ...
        return toDisplayUSD(
          l1Estimate.plus(l2Estimate).shiftedBy(-9).multipliedBy(nativeUSD),
        )
      }
  ...
  feeEstimatesUSD() {
    ...
    const type = 'ethereum'
    const currentSymbol =
      this.store('main.networks', type, chainId, 'symbol') || 'ETH'
