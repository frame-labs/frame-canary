...
describe('sentry', () => {
  describe('#beforeSend', () => {
    ...
    const sendErrorEvents = (numEvents) =>
      [...Array(numEvents).fill(validEvent)].map(sendErrorEvent)
    const simulateException = (ex) =>
      sendErrorEvent({ exception: { values: [ex] } })
    ...
    it('should strip asar paths from stackframe modules on Windows', () => {
      ...
      const stackFrameModules =
        sentryEvent.exception.values[0].stacktrace.frames.map(
          (frame) => frame.module,
        )
