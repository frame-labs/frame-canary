...
beforeEach(() => {
  ...
  connectionObserver = store.observer(() => {
    Object.values(mockConnections).forEach((chain) => {
      const primary = store(
        `main.networks.ethereum.${chain.id}.connection.primary`,
      )
...
afterEach((done) => {
  ...
  const activeConnection = Object.values(mockConnections).find(
    (conn) => conn.connection.connected,
  )
...
Object.values(mockConnections).forEach((chain) => {
  it(`sets legacy gas prices on a new non-London block on ${chain.name}`, (done) => {
    ...
    observer = store.observer(() => {
      const gas = store(
        `main.networksMeta.ethereum.${chain.id}.gas.price.levels`,
      )
  ...
  it(`sets fee market prices on a new London block on ${chain.name}`, (done) => {
    ...
    observer = store.observer(() => {
      ...
      if (gas.fees.maxBaseFeePerGas) {
        ...
        expect(gas.fees.maxPriorityFeePerGas).toBe(
          weiToHex(expectedPriorityFee),
        )
        expect(gas.fees.maxFeePerGas).toBe(
          weiToHex(expectedBaseFee + expectedPriorityFee),
        )

        expect(gas.selected).toBe('fast')
        expect(gas.levels.fast).toBe(
          weiToHex(expectedBaseFee + expectedPriorityFee),
        )
