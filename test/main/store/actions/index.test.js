...
describe('#addNetwork', () => {
  ...
  const updaterFn = (node, update) => {
    if (node !== 'main')
      throw new Error(`attempted to update wrong node: ${node}`)
  ...
  it('adds a network with the correct primary RPC', () => {
    ...
    expect(networks.ethereum['137'].connection.primary.custom).toBe(
  ...
  it('adds a network with the correct default connection presets', () => {
    ...
    expect(networks.ethereum['137'].connection.presets).toEqual({
      local: 'direct',
    })
  })
...
describe('#setBalances', () => {
  ...
  const setBalances = (updatedBalances) =>
    setBalancesAction(updaterFn, owner, updatedBalances)
...
describe('#removeBalance', () => {
  ...
  it('removes a balance from all accounts', () => {
    ...
    expect(
      balances['0xd0e3872f5fa8ecb49f1911f605c0da90689a484e'],
    ).not.toContainEqual(
      expect.objectContaining({ address: testTokens.zrx.address }),
    )
    expect(balances['0xd0e3872f5fa8ecb49f1911f605c0da90689a484e']).toHaveLength(
      1,
    )
  })
})
...
describe('#addCustomTokens', () => {
  ...
  const addTokens = (tokensToAdd) =>
    addCustomTokensAction(updaterFn, tokensToAdd)
...
describe('#removeCustomTokens', () => {
  ...
  const removeTokens = (tokensToRemove) =>
    removeTokensAction(updaterFn, tokensToRemove)
...
describe('#addKnownTokens', () => {
  ...
  const addTokens = (tokensToAdd) =>
    addKnownTokensAction(updaterFn, account, tokensToAdd)
...
describe('#setScanning', () => {
  ...
  const setScanning = (scanning) =>
    setScanningAction(updaterFn, owner, scanning)
...
describe('#activateNetwork', () => {
  ...
  const activateNetwork = (type, chainId, active) =>
...
describe('#setBlockHeight', () => {
  ...
  const updaterFn = (node, chainId, update) => {
    ...
    main.networksMeta.ethereum[chainId] = update(
      main.networksMeta.ethereum[chainId],
    )
  }
