...
describe('#getTokenBalances', () => {
  ...
  describe('using multicall', () => {
                  ...
                  returnValues: [
                    tc.returns[0](
                      ethers.BigNumber.from(
                        onChainBalances[token.address].toString(),
                      ),
                    ),
                  ],
                }
    ...
    it('loads token balances for multiple chains', async () => {
      const tokenBalances = await balancesLoader.getTokenBalances(
        ownerAddress,
        knownTokens,
      )
      ...
      expect(tokenBalances).toEqual([
        {
          ...aaveUsdcToken,
          balance: addHexPrefix(
            padToEven(new BigNumber('6245100000').toString(16)),
          ),
        ...
        },
        {
          ...zrxToken,
          balance: addHexPrefix(
            padToEven(new BigNumber('756578458984500000000').toString(16)),
          ),
        ...
        },
        {
          ...badgerDaoToken,
          balance: addHexPrefix(
            padToEven(new BigNumber('17893000000000000000').toString(16)),
          ),
...
function respondToTokenCall(payload) {
  ...
  const token = knownTokens.find(
    (token) =>
      token.address === payload.params[0].to &&
      token.chainId === parseInt(payload.chainId),
  )
  ...
  if (balance) {
    ...
    return (
      callResponse.slice(0, callResponse.length - hexBalance.length) +
      hexBalance
    )
  }
