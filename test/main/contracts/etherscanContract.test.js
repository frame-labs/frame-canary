...
describe('#fetchEtherscanContract', () => {
  ...
  const chains = [
    {
      ...
      apiKey: '3SYU5MW5QK8RPCJV1XVICHWKT774993S24',
    },
    {
      chainId: '0x89',
      domain: 'api.polygonscan.com',
      apiKey: '2P3U9T63MT26T1X64AAE368UNTS9RKEEBB',
    },
    {
      chainId: '0xa',
    ...
    },
    {
      ...
      apiKey: 'VP126CP67QVH9ZEKAZT1UZ751VZ6ZTIZAD',
    },
  ]
  ...
  chains.forEach((chain) => {
    ...
    it(`retrieves a contract from ${name}`, async () => {
      ...
      return expect(
        fetchEtherscanContract(contractAddress, chain.chainId),
  ...
  it('does not retrieve a contract from etherscan when the request fails', async () => {
    ...
    return expect(
      fetchEtherscanContract(contractAddress, '0x1'),
  ...
  it('does not retrieve a contract from an unsupported chain', async () => {
    return expect(
      fetchEtherscanContract(contractAddress, '0x4'),
    ).resolves.toBeUndefined()
  })
