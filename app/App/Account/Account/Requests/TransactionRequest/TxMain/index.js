...
class TxSending extends React.Component {
  ...
  render() {
    ...
    const ensName =
      req.recipient && req.recipient.length < 25 ? req.recipient : ''
          ...
          <div className="_txMainValues">
            <div className="_txMainTransferring">
              <div className="_txMainTransferringPart _txMainTransferringPartLarge">
                <span className="_txMainTransferringSymbol">
                  {currentSymbol}
                </span>
                <span className="_txMainTransferringAmount">
                  {displayValue}
                </span>
              </div>
              <div className="_txMainTransferringPart">
                ...
                <span className="_txMainTransferringEqAmount">
                  {(displayValue * etherUSD).toFixed(2)}
                </span>
              </div>
            </div>
            {address && req.recipientType === 'contract' ? (
              <div className="_txMainTag">{`to contract on ${chainName}`}</div>
            ) : address ? (
              <div className="_txMainTag">{`to account on ${chainName}`}</div>
            ) : null}
            {address ? (
