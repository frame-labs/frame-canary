...
class TransactionRequest extends React.Component {
  ...
  hexToDisplayValue(hex) {
    return (
      Math.round(parseFloat(utils.fromWei(hex, 'ether')) * 1000000) / 1000000
    ).toFixed(6)
  }
  ...
  renderAdjustFee() {
    ...
    return <AdjustFee {...this.props} req={req} />
  }
  ...
  renderTx() {
    ...
    if (mode !== 'monitor' && req.data.nonce) {
      ...
      existingNonces.forEach((nonce, i) => {
        if (req.data.nonce === nonce) {
          ...
          if (
            monitorFilter[i].status === 'confirming' ||
            monitorFilter[i].status === 'confirmed'
          ) {
            txMeta.possible = false
            txMeta.notice = 'nonce used'
          } else if (
            req.data.gasPrice &&
            parseInt(monitorFilter[i].data.gasPrice, 'hex') >=
              parseInt(req.data.gasPrice, 'hex')
          ) {
            txMeta.possible = false
            ...
            Math.ceil(
              parseInt(monitorFilter[i].data.maxPriorityFeePerGas, 'hex') * 1.1,
            ) > parseInt(req.data.maxPriorityFeePerGas, 'hex') &&
            Math.ceil(
              parseInt(monitorFilter[i].data.maxFeePerGas, 'hex') * 1.1,
    ...
    const requiredApproval =
      showWarning && (req.approvals || []).filter((a) => !a.approved)[0]
          ...
          <div className="approveTransaction">
            ...
            <div className="approveTransactionPayload">
              <div className="_txBody">
                ...
                <TxRecipient
                  i={3 + recognizedActions.length}
                  {...this.props}
                  req={req}
                />
                <TxFeeNew
                  i={4 + recognizedActions.length}
                  {...this.props}
                  req={req}
                  chain={chain}
                />
