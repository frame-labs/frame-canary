...
class BalancesPreview extends React.Component {
    ...
    const balances = rawBalances
      ...
      .map((rawBalance) => {
        ...
        const nativeCurrencyInfo =
          networksMeta[rawBalance.chainId].nativeCurrency || {}
  ...
  render() {
    const { address, lastSignerType } = this.store(
      'main.accounts',
      this.props.account,
    )
    ...
    const filteredBlanaces = allBalances.filter((rawBalance) =>
      this.isFilterMatch(rawBalance),
    )
    const balances = filteredBlanaces.slice(0, 4)

    const lastBalanceUpdate = this.store(
      'main.accounts',
      address,
    ...
    const scanning =
      !lastBalanceUpdate || new Date() - new Date(lastBalanceUpdate) > 1000 * 60
      ...
      <div ref={this.moduleRef} className="balancesBlock">
        ...
        {totalValue.toNumber() > 10000 && hotSigner ? (
          <div
            ...
            onClick={() =>
              this.setState({
                showHighHotMessage: !this.state.showHighHotMessage,
              })
            }
