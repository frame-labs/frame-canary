...
class RequestCommand extends React.Component {
  ...
  sentStatus() {
    ...
    const maxFeePerGas = BigNumber(
      usesBaseFee(req.data) ? req.data.maxFeePerGas : req.data.gasPrice,
      16,
    )
    ...
    const confirmations =
      req.tx && req.tx.confirmations ? req.tx.confirmations : 0
    const statusClass =
      req.status === 'error' ? 'txStatus txStatusError' : 'txStatus'
    ...
    if (req && req.tx && req.tx.receipt && nativeUSD) {
      ...
      const paidGas =
        effectiveGasPrice || (parseInt(type) < 2 ? gasPrice : null)
          ...
          {req && req.tx && req.tx.receipt ? (
            <>
              ...
              <div className="txProgressSuccessItem txProgressSuccessItemCenter">
                ...
                <div className="txProgressSuccessItemValue">
                  <div style={{ margin: '0px 1px 0px 0px', fontSize: '10px' }}>
                    $
                  </div>
                  {feeAtTime || '?.??'}
                </div>
