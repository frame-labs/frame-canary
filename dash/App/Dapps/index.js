...
function getOriginsForChain(chain, origins) {
  const { connectedOrigins, disconnectedOrigins } = Object.entries(
    origins,
  ).reduce(
    (acc, [id, origin]) => {
      if (origin.chain.id === chain.id && !originFilter.includes(origin.name)) {
        const connected =
          ...
          (!origin.session.endedAt ||
            origin.session.startedAt > origin.session.endedAt)

        acc[connected ? 'connectedOrigins' : 'disconnectedOrigins'].push({
          ...origin,
          id,
        })
      }
  ...
  return {
    ...
    disconnected: disconnectedOrigins
      .sort(byLastUpdated)
      .filter(
        (origin) => Date.now() - origin.session.lastUpdatedAt < 60 * 60 * 1000,
      ),
  }
}
...
class _OriginModule extends React.Component {
  ...
  updateRequestRate() {
    ...
    this.setState({
      averageRequests: (origin.session.requests / sessionLengthSeconds).toFixed(
        2,
      ),
    })
  }
      ...
      <div>
        ...
        >
          ...
          <div className="sliceOriginReqs">
            <div className="sliceOriginReqsNumber">
...
class Dapps extends React.Component {
  getEnabledChains() {
    return Object.values(this.store('main.networks.ethereum')).filter(
      isNetworkEnabled,
    )
  }
      ...
      return (
        <div className="cardShow">
          ...
          <div
            ...
            onClick={clearOriginsClickHandler}
          >
            Clear All
          </div>
        </div>
