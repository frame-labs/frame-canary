...
function isDisconnected(type, status, isLoading) {
  ...
  if (type === 'trezor') {
    return (
      (status === 'disconnected' || status.includes('reconnect')) && !isLoading
    )
  }
...
class Signer extends React.Component {
  ...
  renderLoadingLive() {
    if (
      ...
      this.getStatus() === 'deriving live addresses'
    ) {
      const liveAccountLimit = this.store('main.ledger.liveAccountLimit')
      const styleWidth =
        liveAccountLimit === 20 ? 120 : liveAccountLimit === 40 ? 120 : 60
            ...
            .map((i) => {
              return (
                <div
                  ...
                  style={{
                    opacity: i <= this.props.liveAddressesFound ? '1' : '0.3',
                  }}
                />
              )
            })}
            ...
            >
              ...
              {this.state.tPin ? (
                <div
                  ...
                  onMouseDown={this.backspacePin.bind(this)}
                >
  ...
  renderTrezorPhrase(active) {
    const allowsDeviceEntry = (this.props.capabilities || []).includes(
  ...
  statusText() {
    ...
    if (status === 'ok') {
      return (
        <div className="signerStatusText signerStatusReady">
          {'ready to sign'}
        </div>
      )
    } else if (status === 'locked') {
  ...
  nextPage(backwards) {
    ...
    const maxPage =
      Math.ceil(signer.addresses.length / this.state.addressLimit) - 1
  ...
  renderPreview() {
    ...
    return (
      <div
        ...
        style={{ zIndex: 1000 - this.props.index }}
      >
          ...
          <div className="signerDetails">
            <div className="signerIcon">
              {((_) => {
                ...
                if (type === 'aragon')
                  return (
                    <div className="signerIconWrap signerIconSmart">
                      {svg.aragon(28)}
                    </div>
                  )
                if (type === 'lattice')
                ...
                return (
                  ...
                  >
                    ...
                    <div className="signerAccountAddress">
                      {address.substr(0, 11)}{' '}
                      {svg.octicon('kebab-horizontal', { height: 20 })}{' '}
  ...
  renderExpanded() {
    ...
    return (
      <div
        className={'expandedSigner cardShow'}
        style={{ zIndex: 1000 - this.props.index }}
      >
        {this.statusText()}
        {this.props.type === 'lattice' && status === 'pair' ? (
          <div className="signerLatticePair">
            <div className="signerLatticePairTitle">
          ...
          <>
                  ...
                  return (
                    <div
                      ...
                      className={
                        !added
                          ? 'signerAccount'
                          : 'signerAccount signerAccountAdded'
                      }
                      onClick={() => {
                        if (
                          this.store('main.accounts', address.toLowerCase())
                        ) {
                          link.rpc('removeAccount', address, {}, () => {})
                        } else {
                          link.rpc(
                    ...
                    >
                      <div className="signerAccountIndex">
                        {index + 1 + startIndex}
                      </div>
                      <div className="signerAccountAddress">
                        {address.substr(0, 11)}{' '}
            ...
            <div className="signerBottom">
              <div
                ...
                onMouseDown={() => this.nextPage(true)}
              >
                {svg.triangleLeft(20)}
              </div>
              <div className="signerBottomPages">
                {page +
                  1 +
                  ' / ' +
                  Math.ceil(signer.addresses.length / addressLimit)}
              </div>
              <div
                className="signerBottomPageNext"
                onMouseDown={() => this.nextPage()}
              >
                {svg.triangleLeft(20)}
              </div>
            </div>
          ...
          <div className="signerInterface">
            {this.renderTrezorPin(
              this.props.type === 'trezor' && status === 'need pin',
            )}
            {this.renderTrezorPhrase(
              this.props.type === 'trezor' && status === 'enter passphrase',
            )}
          </div>
        ) : loading ? (
          <div className="signerLoading">
