...
function getActiveConnection(primary, secondary) {
  if (
    secondary.on &&
    (!primary.on || (isConnected(secondary) && !isConnected(primary)))
  ) {
    return secondary
  }
...
class ChainModule extends React.Component {
  constructor(props, context) {
    ...
    const { id, type } = props
    const primaryCustom =
      context.store('main.networks', type, id, 'connection.primary.custom') ||
      this.customMessage
    const secondaryCustom =
      context.store('main.networks', type, id, 'connection.secondary.custom') ||
      this.customMessage
  ...
  status(type, id, layer) {
    const { status, network, current } = this.store(
      'main.networks',
      type,
      id,
      'connection',
