...
export function sendMessage(event: string, payload?: any) {
  log.debug(
    `child process with pid ${
      process.pid
    } sending "${event}" event with payload: ${JSON.stringify(payload)}`,
  )
  ...
  if (process.send) {
    ...
    log.error(
      `cannot send to main process from worker! connected: ${process.connected} pid: ${process.pid}`,
    )
  }
}
...
function handleMessageFromParent(message: WorkerProcessCommand) {
  log.debug(
    `child process with pid ${process.pid} received message: ${
      message.command
  ...
  if (messageHandlers[message.command]) {
    ...
    log.warn(
      `child process with pid ${process.pid} received unexpected message: ${message.command}`,
    )
  }
}
