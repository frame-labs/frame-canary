...
function notify(
  title: string,
  body: string,
  action: (event: Electron.Event) => void,
) {
...
export class Accounts extends EventEmitter {
  ...
  constructor() {
    ...
    this.accounts = Object.entries(accountsApi.getAccounts()).reduce(
      (accounts, [id, account]) => {
        accounts[id] = new FrameAccount(
  ...
  async add(address: Address, options = {}, cb: Callback<Account> = () => {}) {
    ...
    this.accounts[address] = new FrameAccount(
      { address, created, options, active: false },
      this,
    )
  }
  ...
  confirmRequestApproval(
    ...
    if (currentAccount && currentAccount.requests[reqId]) {
      ...
      const approval = (txRequest.approvals || []).find(
        (a) => a.type === approvalType,
      )
  ...
  private sendRequest(
    ...
    provider.send(
      { id: 1, jsonrpc: '2.0', ...payload, _origin: frameOriginId },
      cb,
    )
  }
              ...
              if (receiptRes.result && account.requests[id]) {
                ...
                if (!txRequest.feeAtTime) {
                  ...
                  if (network.type === 'ethereum' && network.id === 1) {
                    const ethPrice = store(
                      'main.networksMeta.ethereum.1.nativeCurrency.usd.price',
                    )
                ...
                if (
                  ...
                  Object.keys(account.requests).forEach((k) => {
                    ...
                    if (
                      ...
                      txReq.data.nonce ===
                        (account.requests[id] as TransactionRequest).data.nonce
                    ) {
          ...
          if (newHeadRes.error) {
            ...
            const monitor = async () => {
              if (!this.accounts[account.address]) {
                ...
                return log.error(
                  'txMonitor internal monitor had no target account',
                )
              }
            ...
            const removeSubscription = async (requestRemoveTimeout: number) => {
              ...
              provider.off(
                `data:${targetChain.type}:${targetChain.id}`,
                handler,
              )
              this.sendRequest(
                {
  ...
  setSigner(id: string, cb: Callback<Account>) {
    ...
    if (
      ...
      previouslyActiveAccount.address !== currentAccount.address
    ) {
  ...
  signMessage(address: Address, message: string, cb: Callback<string>) {
    ...
    if (
      address.toLowerCase() !==
      currentAccount.getSelectedAddress().toLowerCase()
    )
  ...
  declineRequest(handlerId: string) {
    ...
    if (currentAccount && currentAccount.requests[handlerId]) {
      ...
      setTimeout(
        () =>
          this.accounts[currentAccount.address] &&
  ...
  remove(address = '') {
    ...
    if (currentAccount && currentAccount.address === address) {
      ...
      const defaultAccount = (Object.values(this.accounts).filter(
        (a) => a.address !== address,
      ) || [])[0]
      if (defaultAccount) {
  ...
  private txFeeUpdate(
    ...
    if (!request || request.type !== 'transaction')
      throw new Error(
        `Could not find transaction request with handlerId ${handlerId}`,
      )
    if (request.locked)
  ...
  setBaseFee(
    ...
    try {
      ...
      // New value
      const newBaseFee = parseInt(
        this.limitedHexValue(baseFee, 0, 9999 * 1e9),
        16,
      )
  ...
  setPriorityFee(
    ...
    try {
      ...
      // New values
      const newMaxPriorityFeePerGas = parseInt(
        this.limitedHexValue(priorityFee, 0, 9999 * 1e9),
        16,
      )
  ...
  setGasPrice(
    ...
    try {
      ...
      const newGasPrice = parseInt(
        this.limitedHexValue(price, 0, 9999 * 1e9),
        16,
      )
  ...
  setGasLimit(
    ...
    try {
      const { currentAccount, maxFeePerGas, gasPrice, txType } =
        this.txFeeUpdate(limit, handlerId, userUpdate)
  ...
  adjustNonce(handlerId: string, nonceAdjust: number) {
    ...
    if (!currentAccount)
      return log.error(
        'No account selected during nonce adjustement',
        nonceAdjust,
      )
        ...
        this.sendRequest(
          ...
          (res: RPCResponsePayload) => {
            if (res.result) {
              ...
              const adjustedNonce = intToHex(
                nonceAdjust === 1 ? newNonce : newNonce + nonceAdjust,
              )
  ...
  lockRequest(handlerId: string) {
    ...
    if (currentAccount && currentAccount.requests[handlerId]) {
      ...
      log.error(
        'Trying to lock request ' +
          handlerId +
          ' but there is no current account',
      )
    }
  }
