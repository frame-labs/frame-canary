...
function updateDerivation(
  ...
  const liveAccountLimit =
    accountLimit ||
    (derivation === Derivation.live ? store('main.ledger.liveAccountLimit') : 0)
...
export default class LedgerSignerAdapter extends SignerAdapter {
  ...
  remove(ledger: Ledger) {
    if (ledger.devicePath in this.knownSigners) {
      log.info(
        `removing Ledger ${ledger.model} attached at ${ledger.devicePath}`,
      )
  ...
  reload(ledger: Ledger) {
    log.info(
      `reloading  Ledger ${ledger.model} attached at ${ledger.devicePath}`,
    )
  ...
  private handleDeviceChanges() {
    ...
    reconnections.forEach((disconnection) =>
      this.handleReconnectedDevice(disconnection),
    )
  ...
  private detectDeviceChanges() {
    ...
    const { pendingDisconnections, reconnections } =
      this.getReconnectedLedgers(ledgerDevices)
  ...
  private getAttachedDevices(connectedDevices: ConnectedDevice[]) {
    ...
    return connectedDevices.filter(
      (device) => !(device.path in this.knownSigners),
    )
  }
      ...
      (resolved, disconnection) => {
        if (
          connectedDevices.some(
            (device) => device.path === disconnection.device.devicePath,
          )
        ) {
