...
export default class LatticeAdapter extends SignerAdapter {
  ...
  open() {
    this.settingsObserver = store.observer(() => {
      ...
      Object.values(this.knownSigners).forEach((lattice) => {
        ...
        if (accountLimit !== lattice.accountLimit) {
          ...
          reloadAddresses =
            reloadAddresses || lattice.addresses.length < lattice.accountLimit
          needsUpdate = true
        }
    ...
    this.signerObserver = store.observer(() => {
      const devices: { [id: string]: LatticeSettings } =
        store('main.lattice') || {}
      ...
      Object.entries(devices).forEach(([deviceId, device]) => {
        ...
        const { deviceName, tag, baseUrl, privKey, accountLimit } =
          getLatticeSettings(deviceId)
