...
const surface = {
  identity: async (address = '', chainId: string) => {
  ...
  decode: async (contractAddress = '', chainId: string, calldata: string) => {
    // Decode calldata
    const contractSources: ContractSource[] = [
      { name: 'ERC-20', source: 'Generic ERC-20', abi: erc20Abi },
    ]
  ...
  recog: async (contractAddress = '', chainId: string, calldata: string) => {
