...
export default function rates(pylon: Pylon, store: Store) {
  ...
  function handleRatesUpdates(updates: RateUpdate[]) {
    ...
    const nativeCurrencyUpdates = updates.filter(
      (u) => u.id.type === AssetType.NativeCurrency,
    )

    if (nativeCurrencyUpdates.length > 0) {
      log.debug(
        `got currency rate updates for chains: ${nativeCurrencyUpdates.map(
          (u) => u.id.chainId,
        )}`,
      )
    ...
    if (tokenUpdates.length > 0) {
      log.debug(
        `got token rate updates for addresses: ${tokenUpdates.map(
          (u) => u.id.address,
        )}`,
      )
  ...
  function setAssets(assetIds: AssetId[]) {
    ...
    log.verbose(
      ...
      assetIds
        .filter((a) => a.type === AssetType.NativeCurrency)
        .map((a) => a.chainId),
    )
    log.verbose(
