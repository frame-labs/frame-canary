...
export default function (store: Store) {
  const storeApi = {
    ...
    getNetwork: (id: number) =>
      (store('main.networks.ethereum', id) || {}) as Network,
  ...
  function attemptRestart() {
    log.warn(
      `balances controller stopped, restarting in ${RESTART_WAIT} seconds`,
    )
    stop()
  ...
  function handleChainBalanceUpdate(
    ...
    balances
      .filter(
        (balance) =>
          (
            currentChainBalances.find((b) => b.chainId === balance.chainId) ||
            {}
  ...
  function handleTokenBalanceUpdate(
    ...
    const changedBalances = balances.filter((newBalance) => {
      const currentBalance = currentTokenBalances.find(
        (b) =>
          b.address === newBalance.address && b.chainId === newBalance.chainId,
      )
    ...
    if (changedBalances.length > 0) {
      ...
      const unknownBalances = changedBalances.filter(
        (b) =>
          ...
          !knownTokens.some(
            (t) => t.address === b.address && t.chainId === b.chainId,
          ),
      )
  ...
  function setAddress(address: Address) {
    if (!workerController) {
      log.warn(
        `tried to set address to ${address} but balances controller is not running`,
      )
      return
    }
