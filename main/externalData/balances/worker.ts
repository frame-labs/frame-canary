...
async function tokenBalanceScan(
  address: Address,
  tokensToOmit: Token[] = [],
  chains?: number[],
) {
  try {
    ...
    const eligibleChains = (chains || (await getChains())).filter(
      chainSupportsScan,
    )
    const tokenLists = eligibleChains.map((chainId) =>
      tokenLoader.getTokens(chainId),
    )
    ...
    const tokenBalances = (
      await balances.getTokenBalances(address, tokens)
...
async function chainBalanceScan(address: string, chains?: number[]) {
  try {
    ...
    const chainBalances = await balances.getCurrencyBalances(
      address,
      availableChains,
    )
