...
export default class BalancesWorkerController extends EventEmitter {
  ...
  constructor() {
    ...
    const workerArgs =
      process.env.NODE_ENV === 'development' ? ['--inspect=127.0.0.1:9230'] : []
    ...
    this.worker.on('message', (message: WorkerMessage) => {
      log.debug(
        `balances controller received message: ${JSON.stringify(message)}`,
      )
  ...
  private isWorkerReachable() {
    return (
      ...
      this.worker.channel &&
      this.worker.listenerCount('error') > 0
    )
  }
  ...
  private sendCommandToWorker(command: string, args: any[] = []) {
    ...
    try {
      if (!this.isWorkerReachable()) {
        log.error(
          `attempted to send command "${command}" to worker but worker cannot be reached!`,
        )
        return
      }
