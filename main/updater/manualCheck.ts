...
export default function (opts?: CheckOptions) {
  log.verbose('Performing manual check for updates', {
    prereleaseTrack: opts?.prereleaseTrack,
  })
    ...
    https
      .get(httpOptions, (res) => {
        ...
        res.on('end', () => {
          ...
          log.debug('Manual check response', {
          ...
          if (res.statusCode != 200 || !contentType.includes('json')) {
            ...
            return reject(
              new Error(
                `invalid response, status: ${res.statusCode} contentType: ${contentType}`,
              ),
            )
          }

          const releases =
            parseResponse(rawData).filter(
              (r) => !r.prerelease || opts?.prereleaseTrack,
            ) || []
            ...
            resolve(
              isNewerVersion
                ? {
                    ...
                    location: latestRelease.html_url,
                  }
                : undefined,
            )
          } else {
