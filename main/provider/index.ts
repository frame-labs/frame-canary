...
const getPayloadOrigin = ({ _origin }: RPCRequestPayload) =>
  storeApi.getOrigin(_origin)
...
export class Provider extends EventEmitter {
  ...
  verifySignature(
    ...
    if (signed.length === 134) {
      ...
      try {
        ...
        return cb(
          new Error('Could not resolve message or actor for smart accoount'),
        )
      }
    }
    ...
    accounts.signTypedData(
      ...
      (err, signature) => {
        if (err) {
          ...
          const sig = signature || ''
          try {
            const recoveredAddress = recoverTypedMessage(
              { data, sig },
              req.version,
            )
  ...
  async fillTransaction(
    ...
    try {
      ...
      const chainConfig =
        this.connection.connections['ethereum'][parseInt(rawTx.chainId)]
          .chainConfig
    ...
    this.fillTransaction(newTx, (err, transactionMetadata) => {
      if (err) {
        ...
        if (from && !isCurrentAccount(from, currentAccount))
          return resError(
            'Transaction is not from currently selected account',
            payload,
            res,
          )
  ...
  getTransactionByHash(
    ...
    const res = (response: any) => {
      if (
        response.result &&
        !response.result.gasPrice &&
        response.result.maxFeePerGas
      ) {
        return cb({
          ...response,
          result: {
            ...response.result,
            gasPrice: response.result.maxFeePerGas,
          },
        })
      }
  ...
  sign(payload: RPCRequestPayload, res: RPCRequestCallback) {
    ...
    if (!isCurrentAccount(from, currentAccount))
      return resError(
        'sign request is not from currently selected account.',
        payload,
        res,
      )
  ...
  signTypedData(
    rawPayload: RPCRequestPayload,
    version: Version,
    res: RPCRequestCallback,
  ) {
    // ensure param order is [address, data, ...] regardless of version
    const orderedParams =
      utils.isAddress(rawPayload.params[1]) &&
      !utils.isAddress(rawPayload.params[0])
        ? [
            rawPayload.params[1],
            rawPayload.params[0],
            ...rawPayload.params.slice(2),
          ]
        : [...rawPayload.params]
    ...
    if (
      version !== 'V4' &&
      signerType &&
      [SignerType.Ledger, SignerType.Lattice, SignerType.Trezor].includes(
        signerType,
      )
    ) {
      const signerName = capitalize(signerType)
      return resError(
        `${signerName} only supports eth_signTypedData_v4+`,
        payload,
        res,
      )
    }
  ...
  ifSubRemove(id: string) {
    return Object.keys(this.subscriptions).some((type) => {
      ...
      const index = this.subscriptions[subscriptionType].findIndex(
        (sub) => sub.id === id,
      )
  ...
  clientVersion(payload: RPCRequestPayload, res: RPCSuccessCallback) {
    res({
      id: payload.id,
      jsonrpc: '2.0',
      result: `Frame/v${packageFile.version}`,
    })
  }
  ...
  private addEthereumChain(
    ...
    if (!nativeCurrency)
      return resError('addChain request missing nativeCurrency', payload, res)
  ...
  private parseTargetChain(payload: RPCRequestPayload): Chain {
    if ('chainId' in payload) {
      ...
      const chainConnection =
        this.connection.connections['ethereum'][chainId] || {}
  ...
  private getAssets(
    ...
    try {
      ...
      return resError(
        { message: (e as Error).message, code: 5901 },
        payload,
        cb,
      )
    }
  ...
  sendAsync(payload: RPCRequestPayload, cb: Callback<RPCResponsePayload>) {
    this.send(payload, (res) => {
      if (res.error) {
        const errMessage =
          res.error.message || `sendAsync error did not have message`
  ...
  send(payload: RPCRequestPayload, res: RPCRequestCallback = () => {}) {
    ...
    if (!targetChain) {
      ...
      return resError(
        { message: `unknown chain: ${payload.chainId}`, code: 4901 },
        payload,
        res,
      )
    }
    ...
    function getCoinbase(payload: RPCRequestPayload, res: RPCRequestCallback) {
      accounts.getAccounts((err, accounts) => {
        if (err)
          return resError(
            `signTransaction Error: ${JSON.stringify(err)}`,
            payload,
            res,
          )
        res({
    ...
    if (
      ...
      const version = (
        underscoreIndex > 3
          ? method.substring(underscoreIndex + 1).toUpperCase()
          : undefined
      ) as Version
    ...
    if (method === 'net_version')
      return respondToJsonRPC(payload.id, targetChain.id, res)
    if (method === 'eth_chainId')
      return respondToJsonRPC(payload.id, intToHex(targetChain.id), res)
