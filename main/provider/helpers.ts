...
export function checkExistingNonceGas(tx: TransactionData) {
  ...
  const existing = requests.filter(
    (r) =>
      r.mode === 'monitor' && r.status !== 'error' && r.data.nonce === nonce,
  )

  if (existing.length > 0) {
    if (tx.maxPriorityFeePerGas && tx.maxFeePerGas) {
      const existingFee = Math.max(
        ...existing.map((r) => r.data.maxPriorityFeePerGas),
      )
...
export function getSignedAddress(
  ...
  if (signature.length !== 65)
    return cb(
      new Error('Frame verifySignature: Signature has incorrect length'),
    )
  let v = signature[64]
  v = v === 0 || v === 1 ? v + 27 : v
...
export function hasPermission(address: string, originId: string) {
  const permissions = store('main.permissions', address) as Record<
    string,
    Permission
  >
