  ...
  return new Promise((resolve, reject) => {
    pixels(image.toPNG(), 'image/png', (err, pixels) => {
      ...
      const color = {
        ...
        backgroundShade: `rgb(${colorArray
          .map((v) => Math.max(parseInt(v) - 5, 0))
          .join(', ')})`,
        backgroundLight: `rgb(${colorArray
          .map((v) => Math.min(parseInt(v) + 50, 255))
          .join(', ')})`,
        text: textColor(
          ...(colorArray.map((a) => parseInt(a)) as [number, number, number]),
        ),
      }
...
function extractSession(l: string) {
  ...
  const ens =
    url.port === '8421' ? url.hostname.replace('.localhost', '') || '' : ''
  return { session, ens }
}
...
async function extractColors(url: string, ens: string) {
  ...
  view.webContents.session.webRequest.onBeforeSendHeaders((details, cb) => {
    ...
    if (
      details.url.includes('127.0.0.1:1248') ||
      details.url.includes('localhost:1248')
    ) {
