...
export default class FrameManager {
  ...
  manageViews(frames: Record<string, Frame>) {
    ...
    frameIds.forEach((frameId) => {
      ...
      if (!frameInstance)
        return log.error('Instance not found when managing views')
      ...
      instanceViewIds
        ...
        .forEach((instanceViewId) =>
          viewInstances.destroy(frameInstance, instanceViewId),
        )
      ...
      frameViewIds.forEach((frameViewId) => {
        ...
        if (!instanceViewIds.includes(frameViewId))
          viewInstances.create(frameInstance, viewData)
  ...
  broadcast(channel: string, args: any[]) {
    Object.keys(this.frameInstances).forEach((id) =>
      this.sendMessageToFrame(id, channel, ...args),
    )
  }
  ...
  refocus(id: string) {
    ...
    if (frameInstance) {
      ...
      if (currentScreen.id !== frameScreen.id)
  ...
  isFrameShowing() {
    return Object.keys(this.frameInstances).some((win) =>
      this.frameInstances[win].isVisible(),
    )
  }
}
