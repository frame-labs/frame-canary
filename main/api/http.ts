  ...
  if (req.method === 'OPTIONS') {
    ...
    req
      ...
      .on('end', async () => {
        ...
        if (
          protectedMethods.indexOf(payload.method) > -1 &&
          !(await isTrusted(payload))
        ) {
          let error = {
          ...
          res.end(
            JSON.stringify({ id: payload.id, jsonrpc: payload.jsonrpc, error }),
          )
        } else {
          if (payload.method === 'eth_pollSubscriptions') {
            ...
            const send = (force: boolean) => {
              ...
              if (result.length || payload.params[1] === 'immediate' || force) {
                ...
                cleanupTimers[id] = setTimeout(
                  cleanup.bind(null, id),
                  20 * 1000,
                )
              } else {
                const sendResponse = () => {
